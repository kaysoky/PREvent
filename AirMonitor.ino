...
//pin change ISR in interrupt vector table -> already defined in software serial
//ISR(PCINT0_vect)  {
//
//}
...
void loop() {
  ...
  uint8_t result = ble112.ble_cmd_attributes_write(GATT_HANDLE_C_TX_DATA,0, 6, (const uint8*)bluetooth_data);
...
void formatBluetoothData()  {
  ...
  bluetooth_data[4] = bluetooth_data[4] + ((voc << 2) & 0xFF);
  bluetooth_data[3] = ((voc >> 6) & B0001111);
  
  bluetooth_data[3] = bluetooth_data[3] + ((humd << 4) & 0xFF);
  bluetooth_data[2] = (humd >> 4) & 0xFF;
  bluetooth_data[1] = (humd >> 12) & B00000011;
  
  bluetooth_data[1] = bluetooth_data[1] + ((temp << 2) & 0xFF);
...
//    delay(I2C_POLLING_DELAY); // Wait to prevent overpolling
...
//  }
  ...
  //Serial.println(data[3] >> 6, BIN);  //prints read status bit
}
